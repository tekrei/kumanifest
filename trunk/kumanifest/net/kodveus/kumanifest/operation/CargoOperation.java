...
public class CargoOperation implements OperationInterface {
	...
	private void rsToCargo(ResultSet rs, Cargo cargo) throws SQLException {
		...
		cargo.setCommodity((Commodity) CommodityOperation.getInstance().get(
				rs.getLong("commodityId")));
		cargo.setContainerId(rs.getLong("containerId"));
		cargo.setImco(rs.getString("imco"));
		cargo.setPack((Pack) PackOperation.getInstance().get(
				rs.getLong("packId")));
	...
	public long create(VeriSinif vs) {
		...
		// Cargo kaydini veritabanina ekleyelim
		String sql = "INSERT INTO cargo (cargoType, commodityId, containerId, imco, packId, unno,netWeight,packTotal) VALUES(";

		sql += "'" + cargo.getCargoType() + "',"
				+ cargo.getCommodity().getCommodityId() + ","
				+ cargo.getContainerId() + ",'" + cargo.getImco() + "',"
				+ cargo.getPack().getPackId() + ",'" + cargo.getUnno() + "',"
				+ cargo.getNetWeight() + "," + cargo.getPackTotal() + ")";
		try {
	...
	public boolean update(VeriSinif vs) {
		...
		sql += " cargoType='" + cargo.getCargoType() + "', commodityId="
				+ cargo.getCommodity().getCommodityId() + ", containerId="
				+ cargo.getContainerId() + ", imco='" + cargo.getImco()
				+ "', packId=" + cargo.getPack().getPackId() + ", unno='"
				+ cargo.getUnno() + "', packTotal=" + cargo.getPackTotal()
				+ ", netWeight=" + cargo.getNetWeight() + " WHERE cargoId="
				+ cargo.getCargoId();
	...
	public VeriSinif next(Long id) {
		Cargo cargo = null;
		String sql = "SELECT * FROM cargo WHERE cargoId>" + id
				+ " ORDER BY cargoId";
		try {
			ResultSet rs = DBManager.getInstance().executeQuery(sql);
